...
def create_table_without_deflection():
    with st.container(key='Specified_Location_cont',
                      border=True):
        col1, col2 = st.columns([2, 1], gap='medium', vertical_alignment='center')

        with col1:
            st.write('### **Values at Specified Location**')
        ...
        st.dataframe(data={f"Location ({units_dict[unit_systems]['length']})": [location_value],
                           f'Shear Force ({units_dict[unit_systems]['force']})': [beam.get_shear_force(location_value)],
    ...
    with st.container(key='Specified_Intervals_cont',
                      border=True):

        col1, col2 = st.columns([2, 1], gap='medium', vertical_alignment='center')

        with col1:
            st.write('### **Values at Specified Intervals**')
        with col2:
...
def create_table_with_deflection():
    ...
    with st.container(key='Specified_Interval_cont_2',
                      border=True):
